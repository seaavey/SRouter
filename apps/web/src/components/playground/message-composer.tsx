...
export function MessageComposer({ input, selectedModel, streaming, onInputChange, onSend, onCancel }: MessageComposerProps) {
    return (
        <div className="border-t border-border bg-muted/20 p-3 sm:p-4">
            <div className="mb-2 flex items-center justify-between gap-3 font-mono text-[10px] uppercase tracking-[0.1em] text-muted-foreground">
                ...
                <span className="normal-case tracking-normal">Enter to send · Shift + Enter for newline</span>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
                <textarea
                    ...
                    placeholder={selectedModel ? `Message ${selectedModel.id}...` : "Select a model first..."}
                ...
                />
                <p id="playground-message-hint" className="sr-only">Press Enter to send. Press Shift and Enter to add a new line.</p>
                {streaming ? (
                    <Button type="button" variant="outline" size="lg" onClick={onCancel} className="sm:min-w-24">
                        <Square />
                        Cancel
                    </Button>
                ) : (
                    <Button type="button" size="lg" onClick={onSend} disabled={!selectedModel || !input.trim()} className="sm:min-w-24">
                        <Send />
                        Send
                    </Button>
                )}
