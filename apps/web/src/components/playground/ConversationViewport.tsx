...
export function ConversationViewport({
    ...
    return (
        ...
        >
            <h2 id="playground-canvas-title" className="sr-only">
                Conversation
            </h2>
            <div
                    ...
                    <div className="flex min-h-[22rem] h-full flex-col justify-between px-5 py-6 sm:px-8 sm:py-8">
                        ...
                        <div className="max-w-lg pb-6">
                            ...
                            <p className="mt-3 max-w-md text-xs leading-relaxed text-muted-foreground">
                                The response will stream here as it arrives. Use Parameters when you
                                need to tune the request.
                            </p>
                        ...
                        {messages.map((message, index) => {
                            ...
                            return (
                                <li
                                    ...
                                    className="grid gap-2 px-5 py-5 sm:grid-cols-[6.5rem_minmax(0,1fr)] sm:gap-5 sm:px-8"
                                >
                                    <div className="flex items-start justify-between gap-3 sm:block">
                                        <span className="font-mono text-[10px] font-medium uppercase tracking-[0.12em] text-muted-foreground">
                                            {message.role}
                                        </span>
                                        <span className="font-mono text-[10px] text-muted-foreground/60 sm:mt-2 sm:block">
                                            {String(index + 1).padStart(2, "0")}
                                        </span>
                                    </div>
                                    <p
                                        className={`max-w-3xl whitespace-pre-wrap text-xs leading-[1.8] ${isError ? "text-destructive" : "text-foreground"}`}
                                    >
                                        {message.content || (
                                        ...
                                        {isActiveAssistant && message.content ? (
                                            <span
                                                className="ml-1 inline-block h-3 w-px translate-y-[2px] bg-foreground align-baseline"
                                                aria-hidden="true"
                                            />
                                        ) : null}
                                    </p>
